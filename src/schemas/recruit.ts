...
const firstRecruitSchema = Joi.object().keys({
...
const secondRecruitSchema = Joi.object().keys({
  introduce: Joi.string().length(255).required(),
});

const thirdRecruitSchema = Joi.object().keys({
...
const fourthRecruitSchema = Joi.object().keys({
